...
public class BikeRental {
    ...
    public static void bookBikeForRent(String username) throws Exception {
        ...
        System.out.println("Enter Bike No to Rent: ");
        ...
        // Check if the bike is available in the bike list
        String checkBikeSql = "SELECT * FROM bike_list WHERE bike_no = ?";
        PreparedStatement pstmt = con.prepareStatement(checkBikeSql);
        pstmt.setInt(1, bike_no);
        ResultSet rs = pstmt.executeQuery();

        if (rs.next()) {
            // Bike is available in the list, now check availability for the date range
            if (isBikeAvailable(bike_no, rentDateFrom, rentDateTo)) {
                // Bike is available for the selected date range
                ...
                insertPaymentStmt.setInt(1, bike_no);
                ...
                System.out.println("Bike booked successfully from " + rentDateFrom + " to " + rentDateTo);
                System.out.println("Total amount to be paid: " + totalAmount);
            } else {
                // Bike is not available for the selected date range, suggest next available date range
                Date nextAvailableFrom = getNextAvailableFromDate(bike_no, Date.valueOf(rentDateTo));
                System.out.println("Bike is not available for the selected date range.");
                System.out.println("The bike is available from " + nextAvailableFrom + " onwards.");
            }
        } else {
            System.out.println("Bike not found in the database.");
        }
    }

    private static boolean isBikeAvailable(int bike_no, LocalDate rentDateFrom, LocalDate rentDateTo) throws SQLException {
        ...
        return !rs.next();  // If no records are returned, the bike is available
    }

    private static Date getNextAvailableFromDate(int bike_no, Date rentDateTo) throws SQLException {
        ...
        if (rs.next() && rs.getDate("last_rent_date") != null) {
            ...
            return new Date(System.currentTimeMillis());  // If no previous rentals, bike is available from today
        }
    }
